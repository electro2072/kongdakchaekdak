...
export const ko = {
  ...
  profile: {
    ...
    statPlaceholder: '–',
    /**
     * 회원 탈퇴 (G16/S8) — design/hifi_mockup_v1.html v1.21 Frame 05.5(확인 다이얼로그)·05.6(완료 토스트).
     * [담백] 선 2 — 삭제·권한 자리라 은유 금지. 보이스 문서 §4 대조표에는 없는 자리 [문서 미수록].
     * 문구는 목업과 2026-09-11 확정안 그대로다. 공지 문서(`ACCOUNT_DELETION.md`)와 어긋나면 안 되는 자리.
     */
    withdraw: {
      /** 프로필 탭 로그아웃 아래 텍스트 링크 — §2 직역 고정 */
      link: '회원 탈퇴',
      dialogTitle: '정말 탈퇴하시겠어요?',
      /** 다이얼로그 본문 1행 (Frame 05.5) */
      dialogDeleteNotice:
        '서재·독서 기록·사진·공유 링크가 모두 삭제되며 복구할 수 없어요.',
      /**
       * 다이얼로그 본문 2행 — 2026-09-11 확정(목업 반영 전).
       * 그룹 API가 아직 미연동이라 모임장 여부를 알 수 없다 → 조건 없이 모든 사용자에게 항상 표시.
       */
      dialogGroupNotice:
        '모임장을 맡은 모임은 가장 먼저 가입한 멤버에게 넘어가고, 혼자인 모임은 삭제돼요.',
      /** 위험(danger) 확인 버튼 — Frame 05.4 "로그아웃"과 같은 표기 규칙 */
      confirm: '탈퇴',
      /** Frame 05.6 — 로그인 화면으로 돌아간 뒤 뜨는 완료 토스트 */
      success: '탈퇴가 완료됐어요',
    },
    edit: {
      changePhoto: '사진 변경',
  ...
  apiError: {
    ...
    /** [담백] 없는 주소(404)·지원하지 않는 방식(405) — 앱과 서버 버전이 어긋날 때만 보인다. 재시도로 안 풀린다. */
    routeNotFound: '앱을 최신 버전으로 업데이트해주세요.',
    methodNotSupported: '앱을 최신 버전으로 업데이트해주세요.',
  },
